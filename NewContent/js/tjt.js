...
var bar = function (id,title,data){
	...
	this.show = function (){
		...
		//起始
		var s_img = [0,-52,-104,-156,-208,0,-52,-104,-156,-208];  //52
		//中间点起始坐标
		var c_img = [-312,-339,-367,-395,-420,-312,-339,-367,-395,-420]; //27
		//结束
		var e_img = [-26,-78,-130,-182,-234,-26,-78,-130,-182,-234];  //52
		var that = this;
		var div;
		$(this.data).each(function(i,n){
			...
			if(img>0)
			{
				div = "<div style='width:3px;border-radius:35px;height:16px;background:url("+that.bgimg+") 0px "+s_img[i]+"px ;float:left;'></div><div fag='"+img+"' style='width:0%;height:16px;background:url("+that.bgimg+") 0 "+c_img[i]+"px repeat-x ;float:left;'></div><div style='width:3px;height:16px;background:url("+that.bgimg+") 0px "+e_img[i]+"px ;float:left;'></div>";
			}
			else
			{
				div='';
